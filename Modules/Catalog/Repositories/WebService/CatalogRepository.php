...
use Modules\Catalog\Entities\Rating;
...
class CatalogRepository
{
    ...
    public function getProductsByCategory($request)
    {
        ...
        if ($request->rate) {
            $products->whereHas('productRatings', function($query) use($request){
    ...
    public function rateProduct($request, $id)
    {
        $product = $this->prd->active();
        ...
        if($product){
            return $this->rating->create([
                'user_id'   => auth('api')->id() ?? ($request->user_token ?? null),
                'product_id'    => $id,
                'rate'      => $request->rate,
            ]);
        }
