...
class TapPaymentService
{
    ...
    {
        $this->apiKey = config('setting.payment_gateway.tap.' . (config('setting.payment_gateway.tap.payment_mode')) . '.API_KEY') ?? 'sk_test_eoIW8Dm6XyTuUdk0qsf71cj9';
    }
    /**
     * @param $order
     * @param $payment
    ...
    {
        $fields = $this->getRequestFields($order,$type,$payment,$userToken);
     ...
     * @return array
     */
    private function getRequestFields($order,$type,$payment,$userToken = '')
    {
		...
		return [
            'amount'               => priceWithCurrenciesCode($order['total'] , true , false),
            'currency'             => priceWithCurrenciesCode($order['total'] , false , true),
            'threeDSecure'         => true,
            'save_card'            => false,
	...
	public function paymentUrls($type)
    {
        switch ($type){
            ...
            case 'api-order':
                return route('api.orders.success.tap');
        }
